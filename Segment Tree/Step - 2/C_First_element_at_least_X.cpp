...
struct segtree{
    ...
    int findrf(int v, int x, int lx, int rx)
    {
        if (rx - lx == 1)
            return lx;
        ...
        if (vals[2*x + 1] >= v)
            return findrf(v, 2*x + 1, lx, m);

        else if (vals[2*x + 2] >= v)
            return findrf(v, 2*x + 2, m, rx);
...
int main() 
{
    ...
    while (m--)
    {
        ...
        else if (op == 2)   //find the minimum index j such that a[j] >= x. If there is no such element, print −1
        {
            int x;
            cin>>x;
